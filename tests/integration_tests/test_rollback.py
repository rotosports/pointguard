...
def custom_ethermint(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-pointguardd.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/pointguardd"
    )
    print(broken_binary)
...
def test_rollback(custom_ethermint):
    ...
    update_node2_cmd(custom_ethermint.base_dir, "pointguardd", 2)
