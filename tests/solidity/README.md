...
**Prerequisite**: in the repo's root, run `make install` to install the `pointguard` and `pointguard` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `ethermint` network. The `test-helper.js` will help you spawn an `pointguard` process before running the tests.
...
If you need to take more control, you can also run `pointguard` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-ethermint`. You should see `pointguard` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `pointguard query tx <cosmos-sdk tx>`
...
Running `pointguard keys list --keyring-backend=test` should output:
