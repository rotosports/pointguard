...
**Prerequisite**: in the repo's root, run `make install` to install the `pointguardd` and `pointguardd` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `ethermint` network. The `test-helper.js` will help you spawn an `pointguardd` process before running the tests.
...
If you need to take more control, you can also run `pointguardd` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-ethermint`. You should see `pointguardd` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `pointguardd query tx <cosmos-sdk tx>`
...
Each with roughly 100 ETH available (1e18 fury).

Running `pointguardd keys list --keyring-backend=test` should output:
