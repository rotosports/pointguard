...
func TestKeyring(t *testing.T) {
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, pointguard.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, pointguard.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := hdwallet.MustParseDerivationPath(pointguard.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Pointguard implementation
	...
	// Equality of Eth and Pointguard implementation
	...
	// Inequality of wrong derivation path of Eth and Pointguard implementation
